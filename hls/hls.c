...
#ifdef __APPLE__
#include <sys/xattr.h>
/* Failsafe macro definitions if Apple's headers still hide them */
#ifndef major
...
char *sniff_magic_bytes(const char *path) {
    ...
    if (n >= 4 && memcmp(buf, "\xca\xfe\xba\xbe", 4) == 0) return strdup("Mach-O Universal");
 ...
 * Layout & Presentation
 * ------------------------------------------------------------------------- */
void print_json(FileEntry *entries, size_t count, const char *dir_path) {
    printf("[\n");
    for (size_t i = 0; i < count; i++) {
        ...
        if (e->stat_ok) {
            printf("    \"inode\": %llu,\n", (unsigned long long)e->sb.st_ino);
            printf("    \"mode_octal\": \"0%o\",\n", e->sb.st_mode & 07777);
            printf("    \"size\": %lld,\n", (long long)e->sb.st_size);
            printf("    \"uid\": %u,\n", e->sb.st_uid);
            printf("    \"gid\": %u,\n", e->sb.st_gid);
            printf("    \"mtime\": %lld,\n", (long long)e->sb.st_mtime);
            
            bool is_dir = S_ISDIR(e->sb.st_mode);
            bool is_sym = S_ISLNK(e->sb.st_mode);
            printf("    \"is_dir\": %s,\n", is_dir ? "true" : "false");
            printf("    \"is_symlink\": %s", is_sym ? "true" : "false");

            if (is_sym && e->link_target) {
                printf(",\n    \"symlink_target\": \"%s\"", e->link_target);
                printf(",\n    \"broken_link\": %s", e->broken_link ? "true" : "false");
            }
        } else {
...
void print_columns(FileEntry *entries, size_t count, const Config *cfg) {
    ...
    for (size_t row = 0; row < num_rows; row++) {
        for (size_t col = 0; col < num_cols; col++) {
            ...
            if (cfg->show_inode) {
                chars_printed += printf("%s%7llu%s ", cfg->colorize ? ANSI_GRAY : "", 
                                        (unsigned long long)e->sb.st_ino, reset);
            }
...
void print_long(FileEntry *entries, size_t count, const Config *cfg) {
    ...
    for (size_t i = 0; i < count; i++) {
        FileEntry *e = &entries[i];
        if (!e->stat_ok) {
            fprintf(stderr, "%shls: cannot access '%s': %s%s\n", 
                    cfg->colorize ? ANSI_RED : "", e->name, strerror(errno), cfg->colorize ? ANSI_RESET : "");
            continue;
        }
        ...
        /* Device node parsing for major/minor IDs */
        if (S_ISCHR(e->sb.st_mode) || S_ISBLK(e->sb.st_mode)) {
            snprintf(size_buf, sizeof(size_buf), "%3d, %3d", major(e->sb.st_rdev), minor(e->sb.st_rdev));
        } else if (cfg->human_sizes) {
        ...
        const char *color = cfg->colorize ? get_color(e) : "";
        const char *reset = cfg->colorize ? ANSI_RESET : "";
        const char *gray = cfg->colorize ? ANSI_GRAY : "";

        /* Extended data prep */
        if (cfg->show_inode) {
            printf("%s%7llu%s ", gray, (unsigned long long)e->sb.st_ino, reset);
        }
        ...
        printf("%s%s ", git_badge, perms);

        if (cfg->octal_perms) {
            printf("%s%04o%s ", gray, e->sb.st_mode & 07777, reset);
        }

        printf("%2hu %-8s %-8s %8s %-13s %s %s%s%s",
        ...
        if (e->link_target) {
            ...
            if (e->broken_link) {
                printf(" %s[BROKEN]%s", cfg->colorize ? ANSI_RED : "", reset);
            }
        }
        if (cfg->sniff_type && e->magic_desc) {
            printf(" %s(%s)%s", gray, e->magic_desc, reset);
        }
        putchar('\n');
...
FileEntry *load_directory(const char *dir_path, const Config *cfg, const GitContext *git_ctx, size_t *out_count) {
    ...
    while ((dirp = readdir(dp)) != NULL) {
        ...
        if (cfg->regex_filter) {
            if (regexec((regex_t *)&cfg->compiled_regex, dirp->d_name, 0, NULL, 0) != 0) {
                continue;
            }
        }
...
void traverse_directory(const char *dir_path, const Config *cfg, bool print_dir_name) {
    ...
    if (!entries) {
        fprintf(stderr, "%shls: cannot open directory '%s': %s%s\n", 
                cfg->colorize ? ANSI_RED : "", dir_path, strerror(errno), cfg->colorize ? ANSI_RESET : "");
        if (cfg->git_status) free_git_context(&git_ctx);
        return;
    }

    if (print_dir_name && !cfg->json_output) {
        printf("%s:\n", dir_path);
    }

    if (cfg->json_output) {
        print_json(entries, count, dir_path);
    ...
    if (cfg->recursive && !cfg->json_output) {
        for (size_t i = 0; i < count; i++) {
            FileEntry *e = &entries[i];
            if (e->stat_ok && S_ISDIR(e->sb.st_mode) && !S_ISLNK(e->sb.st_mode)) {
                putchar('\n');
...
void print_short_help(const char *progname) {
    printf("Usage: %s [-%saFlrRnStHM@GsijOI%s] [-%sE%s regex] [-%sh%s] [-%sm%s] [%sfile%s ...]\n",
           progname, ANSI_BOLD, ANSI_RESET, ANSI_BOLD, ANSI_RESET, ANSI_BOLD, ANSI_RESET, ANSI_BOLD, ANSI_RESET, ANSI_UNDERLINE, ANSI_RESET);
...
void print_manpage(bool colorize) {
    ...
    printf("%sSYNOPSIS%s\n", b, rst);
    printf("     %shls%s [-%saFlrnRStHM@GsijOI%s] [-%sE%s %spattern%s] [-%sm%s] [%sfile%s ...]\n\n", b, rst, b, rst, b, rst, u, rst, b, rst, u, rst);
    ...
    printf("     %s-E%s      Apply internal POSIX regular expression filter to file names.\n", b, rst);
    ...
    printf("     %s-i%s      Print raw inode index numbers (-l tracks them gracefully).\n", b, rst);
    printf("     %s-I%s      Format timestamps explicitly as ISO-8601 (YYYY-MM-DD HH:MM:SS).\n", b, rst);
    printf("     %s-j%s      Emit output as a structured JSON array (ignores rendering flags).\n", b, rst);
    ...
    printf("     %s-O%s      Render raw octal mode integer (e.g. 0755) in long listings.\n", b, rst);
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "alHFtrRnShmGM@TsiOIjE:")) != -1) {
        switch (opt) {
            ...
            case 'n':
                ...
                break;
            case 'i': cfg.show_inode = true;   break;
            case 'O': 
                cfg.octal_perms = true; 
                cfg.long_format = true;
                break;
            case 'I': cfg.iso_time = true;     break;
            case 'j': cfg.json_output = true;  break;
            case 'E':
                cfg.regex_filter = optarg;
                if (regcomp(&cfg.compiled_regex, cfg.regex_filter, REG_EXTENDED | REG_NOSUB) != 0) {
                    fprintf(stderr, "hls: invalid regular expression '%s'\n", cfg.regex_filter);
                    exit(EX_USAGE);
                }
                break;
            case 'h':
    ...
    if (targets_count <= 0) {
        ...
        for (int i = optind; i < argc; i++) {
            ...
            if (i + 1 < argc && !cfg.json_output) putchar('\n');
        }
    }

    if (cfg.regex_filter) {
        regfree(&cfg.compiled_regex);
    }
